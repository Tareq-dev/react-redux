...
module.exports.register = async (req, res, next) => {
  try {
    ...
    db.query(q, [req.body.email], (error, data) => {
      ...
      // return false;
      const q = "INSERT INTO register (email, password, role) VALUES (?)";
      const values = [req.body.email, hashedPassword, (role = "user")];
      db.query(q, [values], (error, data) => {
        ...
        db.query(query, [data.insertId], (error, signInData) => {
          if (error) return res.json(error);
          const token = jwt.sign({ email: signInData[0].email }, "jwtkey");
